...
# Tried: default, weighted, adaptive, adaptive-weighted
...
class AdpMutation:
    ...
    def __call__(self, ind):
        ...
        difference = abs((fitness[1]+fitness[0])/fitness[1])
        return ind + (1-difference)*np.random.normal(size=np.array(ind).shape)
...
if __name__ == '__main__':
    ...
    for fit_gen, fit_name in zip(fit_generators, fit_names):
        ...
        for run in range(REPEATS):
            ...
            pop = evolutionary_algorithm(pop, MAX_GEN, fit, [wover, mutate_ind], tournament_selection, mutate_ind, map_fn=map, log=log)
            mutate_ind.step_size = MUT_STEP
