...
EXP_ID = 'weighted' # the ID of this experiment (used to create log names)
# default, weighted, adaptive, differential, differential-multi, hypervolume-weighted, hypervolume-default
...
class WeightedMutation:
    ...
    def __call__(self, ind):
        difference = abs((ind.fitness[1]-ind.fitness[0])/(ind.fitness[1]+0.001))
        a = ind.x + (1-difference)*np.random.normal(size=ind.x.shape)
        np.clip(a, 0, 1, ind.x)
        return ind

class DiffMutation:

    def __init__(self, step_size, diff_pairs):
        self.step_size = step_size
    ...
    def __call__(self, pop):
        ...
        for p in pop:
            if random.random() > MUT_PROB:
                res = copy.deepcopy(p)
                for _ in range(self.diff_pairs):
                    ind1 = pop[random.randrange(0, len(pop))] 
                    ind2 = pop[random.randrange(0, len(pop))]
                    res.x += (F-self.step_size)*(ind1.x-ind2.x)
                sel_parent = pop[random.randrange(0, len(pop))]
                for i in range(len(sel_parent.x)):

                    if random.random() > (1-CR-self.step_size):
                        res.x[i] = sel_parent.x[i]
                np.clip(res.x, 0, 1, res.x)
                output.append(res)
            else:
                output.append(p)
        return output
...
def evolutionary_algorithm(pop, max_gen, fitness, operators, mate_sel, mutate_ind, *, map_fn=map, log=None, opt_hv = np.product(mu.HYP_REF)):
    ...
    for G in range(max_gen):        
        ...
        # offspring = mutate_ind(mating_pool)
        offspring = mate(mating_pool, operators)
        # mutate_ind.step_size *= 0.95
...
if __name__ == '__main__':
    ...
    for fit_name in fit_names:
        ...
        mutate_ind_weighted = WeightedMutation()
        mutate_diff = DiffMutation(step_size=MUT_STEP, diff_pairs=2)
        ...
        weighted_mut = functools.partial(mutation, mut_prob=MUT_PROB, mutate=mutate_ind_weighted)
        diff_mut = functools.partial(mutation, mut_prob=MUT_PROB, mutate=mutate_diff)
        ...
        for run in range(REPEATS):
            ...
            pop = evolutionary_algorithm(pop, MAX_GEN, fit, [xover, weighted_mut], tournament_selection_NSGA2, mutate_ind_weighted, map_fn=map, log=log, opt_hv=opt_hv)
