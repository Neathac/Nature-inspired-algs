...
EXP_ID = 'differential-adaptive-multi' # the ID of this experiment (used to create log names)
# Tried: differential, differential-multi, differential-adaptive, differential-adaptive-multi
F = 0.8
CR = 0.9
        ...
        for p in pop:
            if random.random() > MUT_PROB:
                ...
                for _ in range(self.diff_pairs):
                    ind1 = pop[random.randrange(0, len(pop))] 
                    ind2 = pop[random.randrange(0, len(pop))]
                    res += (F-self.step_size)*(ind1-ind2)
                sel_parent = pop[random.randrange(0, len(pop))]
                for i in range(len(sel_parent)):
...
if __name__ == '__main__':
    ...
    for fit_gen, fit_name in zip(fit_generators, fit_names):
        ...
        mutate_adp = DiffMutation(step_size=MUT_STEP, diff_pairs=2)
        xover = functools.partial(crossover, cross=one_pt_cross, cx_prob=CX_PROB)
        mut = functools.partial(mutation, mut_prob=MUT_PROB, mutate=mutate_adp)
        diff = functools.partial(diff_mutation_cross, diff_pairs=1)
        ...
        for run in range(REPEATS):
            ...
            pop = evolutionary_algorithm(pop, MAX_GEN, fit, [mutate_adp], tournament_selection, mutate_adp, map_fn=map, log=log)
            mutate_adp.step_size = MUT_STEP
