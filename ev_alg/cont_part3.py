...
EXP_ID = 'lamarck-baldwin' # the ID of this experiment (used to create log names)
# Tried baldwinism, lamarck, lamarck-baldwin
...
class LamarckMutation:
    ...
    def __init__(self, step_size, fit_fun):
        self.fit_fun = fit_fun
        self.step_size = step_size

    def __call__(self, ind):
        return ind - STEP_SIZE*cf.numerical_derivative(self.fit_fun, np.array(ind))
...
def baldwin_fit(ind ,fit_fun):
    return fit_fun(ind - STEP_SIZE*cf.numerical_derivative(fit_fun, ind))
...
if __name__ == '__main__':
    ...
    for fit_gen, fit_name in zip(fit_generators, fit_names):
        ...
        lamarck_mutate_ind = LamarckMutation(step_size=MUT_STEP, fit_fun=fit)
        xover = functools.partial(crossover, cross=one_pt_cross, cx_prob=CX_PROB)
        mut = functools.partial(mutation, mut_prob=MUT_PROB, mutate=lamarck_mutate_ind)
        ...
        for run in range(REPEATS):
            ...
            pop = evolutionary_algorithm(pop, MAX_GEN, baldwin_f, [xover, mut], tournament_selection, mutate_ind, map_fn=map, log=log)
